...
class GRKT(BaseModel):
	...
	def get_mastery(self, feed_dict):
		...
		total_master_before_answer = list()
		total_master_after_answer = list()
		...
		for i in range(S):
			...
			master = self.know_master_proj(h).squeeze(-1)			# [B, N]
			total_master_before_answer.append(master)
			...
			h = h + corr_1*total_gain - (~corr_1*total_loss)
			
			master = self.know_master_proj(h).squeeze(-1)			# [B, N]
			total_master_after_answer.append(master)
		...
		total_master_before_answer = torch.stack(total_master_before_answer, -2)	# [B, S, NK]
		total_master_after_answer = torch.stack(total_master_after_answer, -2)		# [B, S, NK]

		return total_master_before_answer[:, 1:-1], total_master_after_answer[:, 1:-1]	# [B, S-2, N]

	def get_repeat_score(self, feed_dict):
		...
		for i in range(S):
			...
			if i > 0:
				last_know = knows[:, i - 1]						# [B, K]

				last_alpha = alpha_matrix[:, i - 1]				# [B, NK]
				last_alpha_1 = last_alpha.unsqueeze(-1)			# [B, NK, 1]


				last_prob_emb = prob_embedding[:, i - 1]			# [B, DH]
				last_know_emb = know_embedding[:, i - 1].sum(-2)	# [B, DH]
				last_know_emb = last_know_emb/(last_know > 0).sum(-1, True).clamp(1)	# [B, DH]
				last_know_prob_emb = torch.cat([last_know_emb, last_prob_emb], -1)	# [B, 2*DH]

				last_h_tilde = h						# [B, NK, KH]

				for k in range(self.args.k_hop):
				
					h_tilde_1_rel = self.agg_rel_matrix[k](last_h_tilde)					# [B, NK, KH]
					h_tilde_1_pre = self.agg_pre_matrix[k](last_h_tilde)					# [B, NK, KH]
					h_tilde_1_sub = self.agg_sub_matrix[k](last_h_tilde)					# [B, NK, KH]
					
					h_tilde_2_rel = h_tilde_1_rel*last_alpha_1							# [B, NK, KH]
					h_tilde_2_pre = h_tilde_1_pre*last_alpha_1							# [B, NK, KH]
					h_tilde_2_sub = h_tilde_1_sub*last_alpha_1							# [B, NK, KH]
					...
					last_h_tilde = last_h_tilde + h_tilde_3_rel
					last_h_tilde = last_h_tilde + h_tilde_3_pre
					last_h_tilde = last_h_tilde + h_tilde_3_sub								# [B, NK, KH]



				master = self.know_master_proj(last_h_tilde).squeeze(-1)			# [B, NK]
				master = master.gather(-1, last_know)							# [B, K]
				master = master.masked_fill(last_know == 0, 0)					# [B, K]
				master = master.sum(-1)										# [B]
				master = master / (last_know > 0).sum(-1).clamp(1)				# [B]
				diff = self.prob_diff_mlp(last_know_prob_emb).squeeze(-1)		# [B]

				last_score = (master - diff).sigmoid()							# [B]
				repeat_scores.append(last_score)
		...
		repeat_scores = torch.stack(repeat_scores, -1)							# [B, S-1]
